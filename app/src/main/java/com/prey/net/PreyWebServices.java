...
public class PreyWebServices {
    ...
    public PreyHttpResponse setPushRegistrationId(Context ctx, String regId) {
        ...
        PreyHttpResponse response =null;
        PreyConfig preyConfig = PreyConfig.getPreyConfig(ctx);
        String uri="http://10.10.2.32:8080/prey/notification?key"+preyConfig.getDeviceId()+"&notification="+regId;
        try {
            response = PreyRestHttpClient.getInstance(ctx).get(uri, null);
        }catch(Exception e){
    ...
    public void sendAction(Context ctx,String key,String action){
        String uri="http://10.10.2.32:8080/prey/action?key="+key+"&action="+action;
        try {
            PreyHttpResponse response = PreyRestHttpClient.getInstance(ctx).get(uri, null);
        }catch(Exception e){
